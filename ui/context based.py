...
def upload_files():
    ...
    response = query_server_with_context(context)
...
def query_server_with_context(context):
    ...
    try:
        response = openai.ChatCompletion.create(
            model="gpt-3.5-turbo",
            messages=[{"role": "user", "content": context}],  # Use the context from the form
            stream=True,  # If you want the response streamed
        )
